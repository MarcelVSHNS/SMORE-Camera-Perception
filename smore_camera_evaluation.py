...
def main():
    sequence = "4"
    DATAPATH = f"E:/coopscenes/seq_{sequence}"
    ...
    results = pd.DataFrame(columns=[
        "frame_id", "translation", "rotation", "translation_error", "rotation_error"
    ])
    ...
    for datarecord in dataset:
        try:
            for frame in datarecord:
                ...
                rotation_mtx = T_cam_infracam[:3, :3]
                rotation_agents_vector = R.from_matrix(rotation_mtx).as_rotvec()
                rotation_agents_deg = np.linalg.norm(rotation_agents_vector) * 180 / np.pi
                if translation < 30.0 and (rotation_agents_deg < 90.0 or rotation_agents_deg > 300.0):
                    # Ground Truth
                    t_cam_vehicle = frame.vehicle.cameras.STEREO_LEFT.info.extrinsic
                    t_vehicle_infra = frame.vehicle.info.extrinsic
                    t_infracam_infra = frame.tower.cameras.VIEW_1.info.extrinsic
                    ...
                    results.loc[len(results)] = [
                        frame.frame_id, translation_agents, rotation_agents_deg, translation_distance, rotation_angle_deg
                    ]
                    print(f"{frame.frame_id}, {translation_agents}, {rotation_agents_deg}, {translation_distance}, {rotation_angle_deg}")
